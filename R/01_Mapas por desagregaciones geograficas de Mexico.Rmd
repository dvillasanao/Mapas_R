...
Desde 1993, la **Secretaría General del Consejo Nacional de Población (`CONAPO`)** implementa una metodología sistemática para evaluar las disparidades territoriales mediante la construcción del Índice de marginación, basado en datos censales. Este instrumento permite una identificación precisa de las zonas y regiones que presentan mayores necesidades socioeconómicas en el país. El índice proporciona un análisis detallado que distingue entre estados, municipios, localidades y áreas geoestadísticas básicas, considerando factores críticos como el acceso a la educación, la calidad de la vivienda, el nivel de ingresos y la ubicación geográfica de los asentamientos.
...
La metodología implementada utiliza bases de datos que incorporan estimaciones del índice de marginación en diversos niveles de desagregación geográfica censal, facilitando una representación cartográfica comprehensiva de las disparidades socioeconómicas presentes en el territorio nacional.
...
```{r, eval = FALSE}
path = paste0(here::here(),"/img/IME_2020_ggspatial.pdf")
ggexport(p, filename = path, width = 8, height = 6, res = 400, device = "cairo")
```
...
```{r, eval = FALSE}
path = paste0(here::here(),"/img/IMM_2020_geom_sf.pdf")
ggexport(p, filename = path, width = 8, height = 6, res = 400, device = "cairo")
```
...
```
```{r, eval = FALSE}
path = paste0(here::here(),"/img/IML_2020_geom_sf_Chiapas.pdf")
ggexport(list = p, filename = path, width = 8, height = 6, res = 400, device = "cairo")
```
...
```
```{r, eval = FALSE}
path = paste0(here::here(),"/img/IMU_2020_gmap.pdf")
ggexport(p, filename = path, width = 8, height = 6, res = 400, device = "cairo")
```
...
```
```{r, eval = FALSE}
path = paste0(here::here(),"/img/IMU_2020_ZMVM_geom_polygon.pdf")
ggexport(p, filename = path, width = 8, height = 6, res = 400, device = "cairo")
```
...
p
```
![](../img/IME_2020_geom_polygon_etiquetas.png){width="90%" fig-align="center"} 
...
```{r, eval = FALSE}
path = paste0(here::here(),"/img/IME_2020_annotation.pdf")
ggexport(p, filename = path, width = 8, height = 6, res = 400, device = "cairo")
```

![](../img/IME_2020_annotation.png){width="90%" fig-align="center"} 
