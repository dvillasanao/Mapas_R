...
memory.size()
invisible(utils::memory.limit(size = 600000))
...
capas_localidad <- shape_localidad %>%
                    sp::spChFIDs(., str_pad(shape_localidad@data$CVE_GEO, 9, "left", pad = "0")) %>%
                     fortify(., id = c("CVE_GEO")) %>%
                      left_join(., IML_2020 %>% dplyr::select(CVE_GEO, GM_2020), 
                                  by = c("id" = "CVE_GEO")) 
              ...
              fortify(., id = "CVE_GEO") %>%
               right_join(., IMU_2020 %>% dplyr::select(CVE_GEO, ENT, NOM_ENT, MUN, NOM_MUN, LOC, NOM_LOC, GM_2020), 
                 ...
                 mutate(CVE_MUN = paste0(ENT, MUN)) %>%
                  right_join(., ZM_2020 %>% dplyr::select(CVE_MUN, CVE_ZM, NOM_ZM), 
                              by = "CVE_MUN")
```
...
saveRDS(capas_estados, file = paste0(here::here(), "/Output/capa_estados.RDS"))
...
saveRDS(capas_municipio, file = paste0(here::here(), "/Output/capas_municipio.RDS"))
