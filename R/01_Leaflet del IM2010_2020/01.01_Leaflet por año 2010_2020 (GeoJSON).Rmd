...
bounding_box
```
### Opción 1 {.tabset .tabset-fade .tabset-pills}

Se genera un mapa interactivo utilizando leaflet en R, añadiendo diferentes capas de polígonos para los años `2020`, `2015` y `2010`. Cada capa está coloreada según una paleta específica.      

- El mapa tiene un zoom fijo entre 5 y 5, sin controles de zoom.      
- Añade capas de polígonos para los estados en los años 2020, 2015 y 2010, coloreados según una variable específica (`GM_2020`, `GM_2015`, `GM_2010`) con una paleta definida (`mypalette`).    
- Establece límites máximos del mapa basados en el cuadro delimitador de los datos.   
- Cada capa de polígonos tiene características visuales específicas como color de borde, opacidad y grosor de línea.   
...
Se añaden la capas de polígonos al objeto de mapa `map`. Cada llamada a `addPolygons` añade una capa de polígonos correspondiente a un año específico (`2020`, `2015`, `2010`).   
...
Se añade un control de título en la esquina inferior derecha del mapa. Además, se incluye una leyenda que muestra los colores y etiquetas correspondientes a los niveles de marginación, utilizando la paleta definida.     

- `addControl()`: Añade un control de título al mapa en la esquina inferior derecha. title es el objeto div que contiene el título y los estilos definidos anteriormente.    
- `addLayersControl()`: Añade un control de capas al mapa, permitiendo al usuario alternar entre las capas de los años `2020`, `2015` y `2010`.   
- `baseGroups = c("2020", "2015", "2010")`: Especifica los nombres de los grupos base que se mostrarán en el control de capas.   
...
### Opción 2 {.tabset .tabset-fade .tabset-pills}

Se sigue la misma estructura que la **Opción 1**, la única diferencia es que se encuentran reducidos todos los `addPolygons` dentro de un loop, en la que se define de manera previa se genera una función `addPolygonLayer`, que introduce todos los parámetros del addPolygons. De esta manera, hace que no se tengan que desplegar las capas de todos los años, y se realiza todo de manera simplificada.      

#### Se definen los años y los shapefiles    

- `years`: Un vector que contiene los años para los que tenemos datos de shapefiles.  
- Se asigna a la lista `shape_estados` los datos de shapefile correspondientes a cada año.      
...
Se añaden los controles adicionales y una leyenda al mapa de Leaflet.   
...
### Opción 3 {.tabset .tabset-fade .tabset-pills} 

Se sigue la misma estructura que la **Opción 2**, se genera un lista de data frames geoespaciales correspondientes a diferentes años, y para cada data frame, añade varias columnas nuevas: una con IDs únicos, otra con el año correspondiente, otra con una combinación de ID y año.   

**Vector y función**   
- `years`: Se define un vector con los años `2020`, `2015` y `2010`.     
- `generate_ids`: Es una función que genera una secuencia de números que los convierte en caracteres y les agrega ceros a la izquierda para que tengan al menos dos dígitos.       
...
- `ID`: Se genera IDs únicos para cada entidad, utilizando la función `generate_ids`. Por ejemplo, para el primer año (2020), generará IDs del 1 al 32; para el segundo año (2015), del 33 al 64; y para el tercer año (2010), del 65 al 96.      
...
# Función para generar el ID de 1 a 96
