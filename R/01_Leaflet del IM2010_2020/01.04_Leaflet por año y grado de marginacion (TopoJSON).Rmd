...
**Transformación y Manipulación de los Datos**:    

- Las listas de datos se transforman en objetos `JSON` utilizando `fromJSON()`.     
- Se generan IDs únicos y se añaden propiedades `ID`, `ANIO` y `id_year` a cada geometría.      
...
**Estilos de la geometría**:     

- Se define una paleta de colores correspondiente a los diferentes grados de marginación.    
...
```{r mapTopoJSON, cache.lazy=TRUE, eval = FALSE}
map <- map %>%
        htmlwidgets::onRender("
          function(el, x) {
            var myMap = this;
