...
Se convierten los datos `GeoJSON` en datos `TopoJSON`, utilizando la función `geo2topo` del paquete `geojsonio`. Sin embargo, hay un pequeño ajuste que debes hacer para que funcione correctamente
...
for (i in seq_along(years)) {
  for(j in seq_along(grados)){
           map <- addPolygonLayer(map, shape_estados[[i]][[j]], year = years[i], grado = grados[j], paleta = paleta[j])
  }
}
```
  ...
  htmlwidgets::onRender("
    function(el, x) {
      var updateLayers = function () {
        var selectedYear = document.querySelectorAll('.leaflet-control-layers-base input:checked')[0].nextSibling.innerText.trim();
        var selectedGrados = Array.from(document.querySelectorAll('.leaflet-control-layers-overlays input:checked')).map(input => input.nextSibling.innerText.trim());

        // Ocultar todas las capas
        el.leafletMap.eachLayer(function(layer) {
          if (layer.options && layer.options.group) {
            el.leafletMap.removeLayer(layer);
          }
        });

        // Mostrar solo las capas del año seleccionado y los grados seleccionados
        selectedGrados.forEach(function(grado) {
          var group = union(selectedYear, grado);
          el.leafletMap.eachLayer(function(layer) {
            if (layer.options && layer.options.group.includes(selectedYear) && layer.options.group.includes(grado)) {
              el.leafletMap.addLayer(layer);
            }
          });
        });
      };
      updateLayers();
      this.on('baselayerchange', e => updateLayers());
      this.on('overlayadd', e => updateLayers());
      this.on('overlayremove', e => updateLayers());
    }")
                                                   ...
                                                   stroke = TRUE,
                                                    weight = 1,
                                                     dashArray = "1",
                                                      opacity = 1,
                                                       color = "#D4D0BD",
                                                        group = paste0(year, grado)
                                                         layerId = paste(id, year, sep = "_")
                                                )
                             return(map)
}
```
...
for (i in seq_along(years)) {
  for(j in seq_along(grados)){
           map <- addPolygonLayer(map, shape_estados[[i]][[j]], year = years[i], grado = grados[j], paleta = paleta[j], id = str_pad(as.character(id_counter), 2, "left", pad = "0"))
