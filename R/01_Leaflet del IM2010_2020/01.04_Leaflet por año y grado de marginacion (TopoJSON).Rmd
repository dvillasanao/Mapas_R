...
La función `topojson_write()` del paquete `geojsonio` para escribir archivos `GeoJSON` a partir de los datos en las listas.  Este código permitirá generar múltiples archivos GeoJSON, uno para cada combinación de año (`2020`, `2015`, `2010`) y  dentro cada grado de marginación.   
...
La función `writeLines` escribe las líneas de texto de un objeto en un archivo. En este caso, los objetos `TopoJSON`, generar múltiples archivos uno para cada combinación de año (`2020`, `2015`, `2010`) y  dentro cada grado de marginación, donde se escriben directamente en archivos con formato .topojson.   
...
Se asegura que cada shapefile tenga un sistema de coordenadas uniforme. Utilizando la función `map2()` de `purrr` para iterar sobre las listas anidadas de shapefiles y sus años correspondientes. Dentro de la función anidada, se itera sobre cada shapefile y sus grados correspondientes, asignandoles el sistema de coordenadas `CRS "+init=epsg:4326" `a cada shapefile y se transforma a este sistema de coordenadas.      

**Vectores y funciónes**    
...
# Aplicar la transformación a cada elemento en las listas anidadas usando map2
...
`TopoJSON en Leaflet`:  Se leen archivos `TopoJSON` y almacenarlos en listas anidadas según los años y grados especificados. La función `readLines` se utiliza para leer el contenido de los archivos `TopoJSON` y almacenar cada línea del archivo como un elemento en un vector de caracteres dentro de las listas correspondientes a cada año y grado. Esto facilita la organización y el acceso a los datos leídos en un formato estructurado.    
...
## Lista de shapefiles de todos los años
shape_estados <- NULL
shape_estados <- list(shape_estados_2020, shape_estados_2015, shape_estados_2010)
                            ...
                            if (is.data.frame(properties)) {
                                ...
                                properties <- properties %>%
                                                mutate(ID = generate_ids(start_id, end_id),
                                                       ANIO = year,
                                                       id_year = paste0(generate_ids(start_id, end_id), year))
                                shape_file$objects$foo$geometries$properties <- properties
                                start_id <<- end_id + 1
                            }
                            shape_file
                        })
...
shape_estados <- map2(shape_estados, years, function(shape_files, year) {
                      map2(shape_files, seq_along(grados), function(shape_file, grado_idx) {
                        #shape_file$objects$foo$geometries$properties$style = list(fillColor = paleta[grado_idx],
                         #                                   color = "#D4D0BD"
                          #                                  )
                        shape_file$style = list(
                                                fillOpacity = 0.8,
                                                stroke = TRUE,
                                                weight = 1,
                                                dashArray = "1",
...
leaflet() %>% addTopoJSON(shape_estados[[1]][[2]])
```
           ...
           map <- addPolygonLayer(map, 
                                   data = shape_estados[[i]][[j]], 
                                    year = years[i], 
                                     grado = grados[j], 
                                      paleta = paleta[j],
                                       id = paste0(str_pad(id_counter, 2, "left", pad = "0"), years[i])
                                  )
...
# lista de TopoJSONs:
shape_estados_1 <- map(seq_along(years), function(year) {
                      map(grados, function(grado) {
                             fromJSON(get(paste0("shape_estados"))[[year]][[grado]], simplifyVector = FALSE)
                      })
                    })
...
# Aplicar transformaciones a las propiedades de cada geometría
start_id <- 1
shape_estados_1 <- map2(shape_estados_1, years, function(shape_files, year) {
                    map2(shape_files, seq_along(grados), function(shape_file, grado_idx) {
